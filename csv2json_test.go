...
func Test_getFileData(t *testing.T) {
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr { // Asserting wether or not we get the correct error value
				t.Errorf("getFileData() error = %v, wantErr %v", err, tt.wantErr)
				return
			}

			if !reflect.DeepEqual(got, tt.want) { // Asserting whethe or not we get the correct wanted value
